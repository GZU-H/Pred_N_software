...
## Load the dataset
data=pd.read_excel("ALLMOFN.xlsx")
data=data.dropna(axis=0) ## Missing value handling

##Divide the data set
X = data.iloc[:,np.r_[1:18]].values
X = X.astype(np.float64)
Y = data.iloc[:,18].values
Y = Y.astype(np.float64)
X_train, X_test, y_train, y_test = TTS(X, Y, test_size=0.3, random_state=42)

## Apply logarithmic transformation to the "Henry's constant" column, assuming it is named "K"
if 'K' in data.columns:
    data['K'] = np.where(data['K'] > 0, np.log(data['K']), 0)  # Use 0 for K=0, apply log for other case
